...
class snake_conv(nn.Module):
    def __init__(self, dim):
        ...
        # self.conv_0 = nn.Conv2d(dim, dim,3, 1, 1)
        # self.conv3x3 = nn.Conv2d(dim * 3, dim, 3, 1, 1)
...
class space_aware_deform_conv(nn.Module):
    def __init__(self, dim):
        super(space_aware_deform_conv, self).__init__()

        self.offset = nn.Sequential(
            SpatialAttention(),
            nn.Conv2d(dim, 18, kernel_size=3, padding=1),
        )
        self.deform_conv = DeformConv2d(dim, dim, 3,1,1)

    def forward(self, x):
        out = self.deform_conv(x, self.offset(x))
        return out
...
class VRWKV_EPIMix(nn.Module):
    def __init__(self, n_embd, init_mode='fancy',
        ...
        # self.omni_shift = space_aware_deform_conv(dim=n_embd)
        self.omni_shift = snake_conv(n_embd)
        # self.omni_shift = nn.Conv2d(n_embd, n_embd,3,1,1)
    ...
    def forward(self, x, resolution):
        ...
        #     elif j % 4 == 1:
        #         # v.flip(-2) [25 24 23 ...2 1]
        #         v = RUN_CUDA(B, T, C, self.spatial_decay[j] / T, self.spatial_first[j] / T, k.flip(-2), v.flip(-2))
        #     elif j % 4 == 2:
        #         v = v.flip(-2)  # reshape回原始维度
        #         h, w = resolution
        #         k = rearrange(k, 'b (h w) c -> b (w h) c', h=h, w=w)
        #         v = rearrange(v, 'b (h w) c -> b (w h) c', h=h, w=w)        # v [1, 6, 11,  ... 20, 25 ]
        #         v = RUN_CUDA(B, T, C, self.spatial_decay[j] / T, self.spatial_first[j] / T, k, v)
