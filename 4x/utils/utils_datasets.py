...
class TrainSetDataLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(file_name[0], 'r') as hf1, h5py.File(ref_file_name[0], 'r') as hf2:
            ...
            hr_down2 = rearrange(hr_down2, 'h w (an1 an2)->(an1 h) (an2 w)',an1=self.angRes, an2=self.angRes, h=self.patch_size * 2, w=self.patch_size * 2)
            data, label, ref, hr_down2 = augmentation_4(Lr_SAI_y, Hr_SAI_y, ref_y, hr_down2)
            Lr_SAI_y = ToTensor()(data.copy())
            Hr_SAI_y = ToTensor()(label.copy())
            ref_SAI_y = ToTensor()(ref.copy())
            hr_down2 = ToTensor()(hr_down2.copy())
            return Lr_SAI_y, Hr_SAI_y, ref_SAI_y, hr_down2, [Lr_angRes_in, Lr_angRes_out]
