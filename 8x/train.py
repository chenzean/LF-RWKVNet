...
def train(train_loader, device, net, criterion, optimizer, logger):
    ...
    for idx_iter, (data, label, ref_y, hr_down4, hr_down2, data_info) in tqdm(enumerate(train_loader), total=len(train_loader), ncols=70):
        ...
        hr_down4 = hr_down4.to(device)
        hr_down2 = hr_down2.to(device)

        output, out4, out2 = net(data, ref, data_info)
        loss8 = criterion(output, out4, out2, label, hr_down2, hr_down4, data_info)
        loss_total = loss8
...
def test(test_loader, device, net, epoch_dir=None, CenterView_dir=None, mat_file=None, aligned_image_dir=None):
    ...
    for idx_iter, (Lr_SAI_y, Hr_SAI_y, ref_y, Sr_SAI_cbcr, data_info, LF_name) in tqdm(enumerate(test_loader), total=len(test_loader), ncols=70):
        ...
        ref_2d_vol, ref_row_nums, ref_col_nums = crop_patch(ref_y, 1, args.patch_size_for_test *args.scale_factor, spa_bound * 8)
        ...
        # print(time_all)
        # single_sence_time_avg.append(time_all)
